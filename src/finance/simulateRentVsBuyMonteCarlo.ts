 ...
 *   - `winners`: An array of strings ("renter", "buyerFixed", or "buyerVariable") indicating which scenario yielded the highest final net balance (after house sale) for each iteration.
 *   - `values`: (Optional) If `options.values` is `true`, an array of objects containing detailed monthly financial data for each iteration. Each object includes `iteration`, `variable` (e.g., "balance"), `value`, and `month`.
